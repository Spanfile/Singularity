...
fn use_form_error_handler(err: UrlencodedError, req: &HttpRequest) -> actix_web::Error {
    form_error_handler(err, req, use_template)
}

fn rename_form_error_handler(err: UrlencodedError, req: &HttpRequest) -> actix_web::Error {
    form_error_handler(err, req, rename_template)
}

fn delete_form_error_handler(err: UrlencodedError, req: &HttpRequest) -> actix_web::Error {
    form_error_handler(err, req, delete_template)
}

fn form_error_handler<P, T>(err: UrlencodedError, req: &HttpRequest, template_fn: P) -> actix_web::Error
where
    P: Fn(Option<T>) -> ResponseBuilder<'static> + 'static,
{
    warn!("Modify Singularity config POST failed: {}", err);
    warn!("{:?}", req);
...
async fn use_singularity_config_page(
    ...
    Either::Left(display_page(id, db_pool.into_inner(), use_template).await)
}

async fn rename_singularity_config_page(cfg_id: web::Query<ConfigId>, db_pool: web::Data<DbPool>) -> impl Responder {
    let id = cfg_id.id;
    display_page(id, db_pool.into_inner(), rename_template).await
}
...
async fn delete_singularity_config_page(
    ...
    Either::Left(display_page(id, db_pool.into_inner(), delete_template).await)
}
...
async fn submit_rename_form(
    ...
    form_action(id, db_pool, rename_template, move |conn, cfg| {
...
async fn submit_delete_form(
    ...
    Either::Left(form_action(id, db_pool, delete_template, move |conn, cfg| cfg.delete(conn)).await)
}

// this is meant to be used in conjunction with the page template renderer functions below
async fn display_page<F>(id: DbId, db_pool: Arc<DbPool>, template_fn: F) -> impl Responder
where
    F: Fn(Option<&str>) -> ResponseBuilder<'static>,
{
    ...
    .expect("failed to spawn task in blocking thread pool")
    {
        Ok((name, _)) => Either::Left((template_fn)(Some(&name))),
